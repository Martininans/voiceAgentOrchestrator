...
class GenericHttpDriver extends BaseDriver {
    constructor() {
        super(config);
        this.providerName = 'generic-http';
        this.webhookUrl = config.voice.genericHttp?.webhookUrl;
        this.apiKey = config.voice.genericHttp?.apiKey;
        this.headers = config.voice.genericHttp?.headers || {};
        this.timeout = config.voice.genericHttp?.timeout || 30000;
    }

    async initialize() {
        console.log('📞 Initializing Generic HTTP driver...');
        
        if (!this.webhookUrl) {
            console.warn('⚠️  Generic HTTP webhook URL not configured');
            return false;
        }
        ...
        console.log('✅ Generic HTTP driver initialized successfully');
        return true;
    }
    ...
    async validateConfig() {
        ...
        if (missing.length > 0) {
            console.warn(`⚠️  Missing Generic HTTP configuration: ${missing.join(', ')}`);
            return false;
        }
    ...
    async handleInboundCall(payload) {
        console.log('📞 Generic HTTP: Processing inbound call');
    ...
    async handleOutboundCall(payload) {
        console.log('📞 Generic HTTP: Initiating outbound call');
        console.log('Payload:', payload);
    ...
    async textToSpeech(payload) {
        console.log('🔊 Generic HTTP: Processing TTS request');
        console.log('Payload:', payload);
    ...
    async sendSMS(payload) {
        console.log('📱 Generic HTTP: Sending SMS');
        console.log('Payload:', payload);
    ...
    async processSpeech(payload) {
        console.log('🎤 Generic HTTP: Processing speech input');
        console.log('Payload:', payload);
...
module.exports = new GenericHttpDriver(); 